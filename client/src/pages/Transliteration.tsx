...
export default function Transliteration() {
  ...
  const previousWord = () => {
    const previous = (wordIndex - 1 + WORDS.length) % WORDS.length;
    setWordIndex(previous);
    setWordAnswer([]);
    setWordPool(shuffle(WORDS[previous].chunks));
  };
  ...
  const previousSentence = () => {
    const previous = (sentenceIndex - 1 + SENTENCES.length) % SENTENCES.length;
    setSentenceIndex(previous);
    setSentenceAnswer([]);
    setSentencePool(shuffle(SENTENCES[previous].chunks));
  };
  ...
  const previousStory = () => {
    const previous = (storyIndex - 1 + STORIES.length) % STORIES.length;
          ...
          <section className="rounded-3xl border bg-white/80 p-6 shadow-sm md:p-10">
            ...
            <div className="mt-8 text-center">
              <p className="text-sm font-bold uppercase tracking-widest text-muted-foreground">
                Listen in English and build the Telugu sound
              </p>
              ...
              <button
                ...
                className="mt-3 rounded-full bg-primary px-5 py-3 font-bold text-primary-foreground"
              >
            ...
            <div className="mt-8 min-h-20 rounded-2xl border-2 border-dashed border-sky-300 bg-sky-50 p-4 text-center">
              <div className="flex min-h-12 flex-wrap justify-center gap-2">
                {wordAnswer.map((chunk, i) => (
                  <button
                    key={`${chunk}-${i}`}
                    onClick={() => removeWord(i)}
                    className="rounded-xl bg-white px-4 py-2 text-2xl font-bold shadow-sm"
                  >
                    {chunk}
                  </button>
                ))}
              ...
              {wordComplete && (
                <p className="mt-3 font-bold text-green-600">
                  ...
                  Correct! {word.transliteration}
                </p>
              )}
            </div>

            <p className="mt-5 text-center text-sm font-bold text-muted-foreground">
              Drag/tap the Telugu pieces in the correct order
            </p>

            <div className="mt-4 flex flex-wrap justify-center gap-3">
              {wordPool.map((chunk, i) => (
                <button
                  draggable
                  key={`${chunk}-${i}`}
                  onDragEnd={() => dragWord(chunk)}
                  onClick={() => dragWord(chunk)}
                  className="rounded-2xl bg-white px-5 py-3 text-2xl font-bold shadow-md hover:-translate-y-1"
                >
                  {chunk}
                </button>
              ))}
